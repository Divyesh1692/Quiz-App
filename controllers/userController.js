...
const userQuizzes = async (req, res) => {
  try {
    const quizzes = await Quiz.find({ "submission.user": req.user.id });
    res
      .status(200)
      .json({ message: "User quizzes fetched successfully", quizzes });
  } catch (error) {
    res
      .status(500)
      .json({ message: "error fetching user quizzes", error: error.message });
  }
};

module.exports = { signup, login, logout, userQuizzes };
